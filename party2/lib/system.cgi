...
sub write_user {
	...
	# -------------------
	# 変数追加する場合は半角スペースか改行を入れて追加(順不同、並べ替え可(login_time以外))
	my @keys = (qw/
	...
	/);
	# ログインした時間　最終ログイン日　名前　パスワード　IP　Host　lib　待ち時間　拘束時間
	# 最終ログイン時間　参加クエスト　滞在家　ギルド　転職回数　レベル経験値　お金　小さなメダル　福引券　レアポイント
	# 疲労度　性別　アイコン　色　職業　ＳＰ　前職業　ＳＰ　最大HP　HP　最大MP　MP　攻　守　素　武器　防具　道具
	# オーブ　預かり所満杯フラグ　宝取得フラグ　飲食フラグ　プレイヤー撃退数　モンスター撃退数　カジノ熟練度　勇者熟練度　魔王熟練度　錬金数　手助け数　ダンジョンイベント　錬金レシピ　メッセージ
