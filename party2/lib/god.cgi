...
# 	['ねがいごと名',				'補足説明',			sub{ 処理($mesに何か文字を入れるとキャンセル)  }],	
	['強くなりたい',				'全ステータス 40 アップ',	sub{ for my $k (qw/mhp mmp at df ag/) { $m{$k}+=40; };	}],
	['スキルを覚えたい',			'Sp 50 アップ',		sub{ $m{sp}     += 50;		}],
	...
	['ギルドランクをあげたい',		'1000 ポイント',		sub{ return unless $m{guild}; &regist_guild_data('point', 1000, $m{guild});	}],
	['ギルドをゴージャスにしたい',	'ギルドが…',		sub{ return unless $m{guild}; my $gid = unpack 'H*', $m{guild}; return unless -f "$guilddir/$gid/log_member.cgi"; open my $fh, ">> $guilddir/$gid/log_member.cgi" or &error("$guilddir/$gid/log_member.cgiファイルが開けません"); print $fh "$time<>1<>金メダル<>0<>etc/win_medal3.gif<>$npc_color<>\n"; close $fh; &regist_guild_data('bgimg', 'god.gif', $m{guild});		}],
	...
	['オルテガを生き返らして',		'自分の家に…',		sub{ open my $fh, ">> $userdir/$id/home_member.cgi" or &error("$userdir/$id/home_member.cgiファイルが開けません"); print $fh "$time<>1<>オルテガ<>0<>chr/029.gif<>$npc_color<>\n"; close $fh;		}],
	...
	['錬金レシピがほしい',			'アイテム',			sub{ &send_item( $m, 3, int(rand(2)+128) );	}],
	['素敵な恋人がほしい',			'恋人が…',			sub{ $mes = 'それは無理な願いだ…。アドバイスとしては積極的にアピールするのだ…';		}],

	# シークレット
	['メイドを雇いたい',			'お世話係',			sub{ open my $fh, ">> $userdir/$id/home_member.cgi" or &error("$userdir/$id/home_member.cgiファイルが開けません"); print $fh "$time<>1<>メイド<>0<>chr/026.gif<>$npc_color<>\n"; close $fh;		}],
);
