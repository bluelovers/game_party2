...
sub casino_html {
	...
	while (my $dir_name = readdir $dh) {
		...
		close $fh;
		my $bet_name = $stage eq 'ドッペル' ? "賭けコイン<b>$bet</b>枚" : "レート<b>$bet</b>";
...
<form method="$method" action="$script">
	...
	<table class="table1">
		...
		<tr><td>進行速度：</td><td>$speed_select</td></tr>
		<tr><td>参加人数：</td><td>$join_select</td></tr>
		<tr><td>賭けコイン：</td><td><input type="text" name="bet" class="text_box_s" style="text-align: right;" value="$min_bet" />枚</td></tr>
		<tr><td>合言葉：</td><td><input type="text" name="p_pass" class="text_box_s" />　</td></tr>
...
sub check_create_casino {
	...
	if ($p_name eq 'ドッペル') {
		$mes = "賭けコインは最低でも $min_bet 枚必要です"	if $in{bet} < $min_bet;
		$mes = "賭けコインは最低でも 1 枚必要です"			if $in{bet} < 1;
		$mes = "賭けコインが足りません"					if $in{bet} > $m{coin};
	}
	else {
		$mes = "レートが異常です"							if $in{bet} < 0 || $in{bet} > @rates;
		$in{bet} = $rates[$in{bet}];
		$mes = "レート$in{bet}で遊ぶためのコインが足りません"	if $m{coin} < $in{bet} * 5;
	}
...
sub _create_room {
	...
	my $bet_name = $in{stage} eq 'ドッペル' ? "賭けコイン$in{bet}枚" : "レート$in{bet}";
...
sub add_member {
	...
	$mes = "$p_nameに参加するためのコインが足りません"		if $bet > $m{coin};
...
sub koukan {
	...
	my $p = qq|<table class="table1"><tr><th>賞品</th><th>コイン</th></tr>|;
	for my $i (1 .. $#prizes) {
